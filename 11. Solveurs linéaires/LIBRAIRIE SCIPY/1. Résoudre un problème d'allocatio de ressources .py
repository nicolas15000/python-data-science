...
en tenant compte des conditions suivantes:
    ...
    Le bénéfice par unité de produit est de 20 $, 12 $, 40 $ et 25 $ pour le premier, deuxième, troisième et 
    
    quatrième produit, respectivement.

    En raison des contraintes de main-d’œuvre, le nombre total d’unités produites par jour ne peut pas dépasser cinquante.

    Pour chaque unité du premier produit, trois unités de la matière première A sont consommées. 
    ...
    En raison des contraintes de transport et de stockage, l'usine peut consommer jusqu'à cent unités de 
...
La fonction objectif (profit) est définie dans la condition 1. La contrainte de main-d'œuvre découle de la condition 2. 
...
Contrairement à l'exemple précédent, vous ne pouvez pas visualiser celui-ci de manière pratique 
...
Dans ce didacticiel, vous allez utiliser deux packages Python pour résoudre le problème de programmation linéaire décrit ci-dessus:
...
SciPy est simple à installer. Une fois que vous l'avez installé, vous aurez tout ce dont vous avez besoin pour démarrer. Son sous-paquet scipy.optimize peut être utilisé à la fois pour l'optimisation linéaire et non linéaire.

PuLP vous permet de choisir des solveurs et de formuler des problèmes de manière plus naturelle. Le solveur par défaut utilisé par PuLP est COIN-OR Branch and Cut Solver (CBC). Il est connecté au solveur de programmation linéaire COIN-OR (CLP) pour les relaxations linéaires et à la bibliothèque de générateurs de coupes COIN-OR (CGL) pour la génération de coupes.

Un autre excellent solveur open source est le GNU Linear Programming Kit (GLPK). Certaines solutions commerciales et propriétaires bien connues et très puissantes sont Gurobi, CPLEX et XPRESS.

En plus d'offrir une flexibilité lors de la définition des problèmes et la possibilité d'exécuter divers solveurs, PuLP est moins compliqué à utiliser que des alternatives comme Pyomo ou CVXOPT, qui nécessitent plus de temps et d'efforts pour maîtriser.
 """
